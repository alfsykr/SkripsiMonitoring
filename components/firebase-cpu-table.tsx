...
export const FirebaseCPUTable = React.memo(() => {
  const {
    firebaseCPUData,
    isFirebaseConnected,
    lastFirebaseUpdate,
  ...
  // Memoized limited data for better performance (max 50 records)
  const limitedData = useMemo(() => {
    return firebaseCPUData.slice(0, 50);
  }, [firebaseCPUData]);

  // Memoized status colors
  const statusColors = useMemo(() => {
    return limitedData.map(laptop => getStatusColor(laptop.status));
  }, [limitedData]);
        ...
        {isFirebaseConnected && (
          <div className="text-sm text-muted-foreground">
            ...
            <p className="text-xs text-muted-foreground/70">
              Showing {limitedData.length} of {firebaseCPUData.length} records
            </p>
          </div>
        )}
      </CardHeader>
              ...
              {limitedData.map((laptop, index) => (
                <TableRow key={laptop.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant="secondary"
                      className={`text-xs ${statusColors[index]}`}
                    >
                      {laptop.status}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-sm">{formatDate(laptop.tanggal)}</TableCell>
...
FirebaseCPUTable.displayName = 'FirebaseCPUTable';
